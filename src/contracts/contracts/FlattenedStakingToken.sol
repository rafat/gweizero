 ...
 * @dev Flattened contract for testing GweiZero optimization
 *      Includes: ERC20, Ownable, Pausable (all flattened below)
 * 
...
contract StakingToken is ERC20, Ownable, Pausable {
    ...
    function batchStake(address[] memory users, uint256[] memory amounts) external onlyOwner {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            require(balanceOf(users[i]) >= amounts[i], "Insufficient balance");
    ...
    function unstake() external whenNotPaused {
        ...
        require(block.timestamp >= userStake.stakedAt + userStake.lockPeriod, "Still locked");
    ...
    function calculateReward(address user) public view returns (uint256) {
        ...
        uint256 baseReward = (userStake.amount * rewardRate * stakingDuration) / (365 days * 100);

        // Tier bonus based on stake size
        if (userStake.amount >= 10000 * 1e18) {
            baseReward = (baseReward * 150) / 100; // 50% bonus
        } else if (userStake.amount >= 1000 * 1e18) {
